...
Once the prototype has answered its question, capture the answer. If the task includes implementation, lift the validated reducer, machine, or function set into the real module. Preserve the HTML shell as a primary source only when the user or repository workflow asks for durable prototype history; a throwaway branch or commit is optional and should not be created implicitly.
