...
When retries can duplicate side effects, read [`ASYNC_AND_WORKFLOWS.md`](ASYNC_AND_WORKFLOWS.md) to choose the retry owner and safety guarantee.
...
Use this for operation-level retries over typed provider errors. For Effect `HttpClient`-level 429 handling and proactive pacing, read [`EFFECT_HTTP_CLIENTS.md`](EFFECT_HTTP_CLIENTS.md).
...
- In tests, control time with `TestClock`; read [`EFFECT_TESTING.md`](EFFECT_TESTING.md).
