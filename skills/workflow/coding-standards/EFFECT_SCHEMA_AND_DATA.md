...
- Apply [`BOUNDARIES_AND_PARSING.md`](BOUNDARIES_AND_PARSING.md) to boundary ownership and trust decisions. Decode unknown input with `Schema.decodeUnknownEffect(...)` by default.
...
Apply [`DOMAIN_MODELING.md`](DOMAIN_MODELING.md) to decide domain optionality. Represent the encoded contract precisely:
...
Apply [`DOMAIN_MODELING.md`](DOMAIN_MODELING.md) to decide which values require brands or refinements.
...
Apply [`ERROR_HANDLING.md`](ERROR_HANDLING.md) to the error's meaning, granularity, context, message, and recovery guidance. `Schema.TaggedErrorClass` is the explicit class exception for typed Effect errors.
...
Every changed Effect data model uses the selected record or variant representation; every reused field preserves the same meaning; every encoded optional or nullish state is intentional; every default produces a required normalized value; every decoder matches its boundary's trust and failure policy; and every serialized error surface follows [`ERROR_HANDLING.md`](ERROR_HANDLING.md).
