...
description: "Create or extend feature-by-feature product behavior documentation from source code, tests, and live verification. Use when the user requests a product-description or behavior-spec repository describing what users see and can do."
---

# Product description repo

Work in the requested documentation location. The commit messages below are suggested groups: initialize Git or commit only when the user requests it or an authorized repository workflow includes it. Otherwise leave the documents ready for review. Create CLAUDE.md only when explicitly requested.
...
5. **Where the documents go.** Use the requested directory or the repository's documentation convention. A separate Git repository is optional; confirm its location when one is requested.
...
4. **AGENTS.md** with the one-line content shown under Outputs, following the `agents-md` skill. Add CLAUDE.md only when explicitly requested.
...
1. **Pilot.** One small, self-contained feature with a real interaction in it (`product-kinds.md` names one per product kind). Use `document-template.md` to establish the depth and structure later documents follow. Cover relevant phases, variants, interrupts, and cross-cutting behavior; mark inapplicable items rather than inventing detail to meet a line count.
...
Delegate independent documents when delegation is available and authorized; otherwise draft them sequentially. Assign shared glossary edits to one coordinator so workers do not overwrite each other's additions.
