...
description: "Periodic pass that keeps a project's Pi verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one PR of proven corrections. Use for /skill:maintain-verification-skill or \"audit the verify skill\"."
disable-model-invocation: true
---

# Maintain a verification skill

A feature map rots the moment the app changes. This skill is the upkeep loop for a skill generated by `/skill:create-verification-skill` (or any project-local Pi verification skill under `.agents/skills/` with a feature map). The unit of rigor is the feature, not every sentence: cover every feature file from source and exercise every feature live, without terminalising every bullet.
...
0. **Locate the target.** Find the project-local Pi verification skill whose body has launch/drive sections and a feature map, usually `.agents/skills/verify-*/`. Several candidates → ask which one; none → stop and point at `/skill:create-verification-skill` instead of inventing a target.
