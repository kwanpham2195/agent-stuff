...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("handoff", {
    ...
    handler: async (args, ctx) => {
      ...
      const action = await ctx.ui.select("What next?", [
        "Open in new session",
        "Save to handoffs",
        "Copy to clipboard",
      ]);
      ...
      if (action === "Save to handoffs") {
        ...
        await writeFile(filepath, editedPrompt, "utf-8");
        ctx.ui.notify(`Saved: ~/.pi/handoffs/${filename}`, "info");
        return;
      }
